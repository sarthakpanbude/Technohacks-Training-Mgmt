        ...
        <div class="d-flex gap-2">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end shadow border-0 rounded-4 p-2">
                    <li><a class="dropdown-item rounded-3" href="actions/export_enrollment.php?format=excel&<?php echo $_SERVER['QUERY_STRING']; ?>"><i class="fas fa-file-excel me-2 text-success"></i>Export to Excel</a></li>
                    <li><a class="dropdown-item rounded-3" href="#" onclick="window.print()"><i class="fas fa-file-pdf me-2 text-danger"></i>Export to PDF</a></li>
                </ul>
            </div>
        </div>
    </div>
        ...
        <form method="GET" class="row g-3 align-items-center">
            <div class="col-md-6">
                <div class="input-group">
                    ...
                    <input type="text" name="search" class="form-control border-start-0 rounded-end-pill" placeholder="Search by name, enrollment or phone..." value="<?php echo htmlspecialchars($search); ?>">
                </div>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3 d-flex gap-2">
                ...
                <a href="enrollment_review.php" class="btn btn-light rounded-pill px-3" title="Reset"><i class="fas fa-redo"></i></a>
            </div>
        </form>
    </div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="py-3">
                            <div class="small">
                                <div class="text-success fw-bold">Paid: ₹<?php echo number_format($s['paid_fee'] ?? 0, 0); ?></div>
                                <div class="text-danger">Pending: ₹<?php echo number_format($s['pending_fee'] ?? 0, 0); ?></div>
                            </div>
                        </td>
                        <td class="py-3">
                        ...
                        <td class="py-3">
                            ...
                            if ($s['admission_status'] == 'pending') $badgeClass = 'bg-warning';
                            ?>
                            <span class="badge <?php echo $badgeClass; ?> bg-opacity-10 text-<?php echo str_replace('bg-', '', $badgeClass); ?> px-3 py-2 rounded-pill text-capitalize">
                        ...
                        <td class="py-3 text-center">
                            <div class="d-flex justify-content-center gap-2">
                                <div class="btn-group shadow-sm rounded-pill overflow-hidden bg-white border">
                                    <a href="generate_receipt.php?id=<?php echo $s['enrollment_no']; ?>" target="_blank" class="btn btn-sm btn-white border-0 px-3" title="Print Receipt">
                                        <i class="fas fa-print text-primary"></i>
                                    </a>
                                    <a href="generate_form.php?id=<?php echo $s['enrollment_no']; ?>" target="_blank" class="btn btn-sm btn-white border-0 px-3 border-start" title="Download PDF">
                                        <i class="fas fa-file-pdf text-danger"></i>
                                    </a>
                                    <?php
                                    $wa_msg = urlencode("Hello " . $s['full_name'] . ", your enrollment is confirmed. Receipt: " . ($s['receipt_no'] ?? 'N/A'));
                                    ?>
                                    <a href="https://wa.me/<?php echo $s['phone']; ?>?text=<?php echo $wa_msg; ?>" target="_blank" class="btn btn-sm btn-white border-0 px-3 border-start" title="WhatsApp Share">
                                        <i class="fab fa-whatsapp text-success"></i>
                                    </a>
                                </div>
                                                                <?php if ($s['admission_status'] != 'active'): ?>
                                    <a href="assign_batch.php?student_id=<?php echo $s['main_student_id']; ?>" class="btn btn-success btn-sm rounded-pill px-3 shadow-sm">
...
@media print {
    .sidebar, .topbar, .filter-bar, .btn-group, .dropdown, .card.border-0.shadow-sm.p-3.mb-4, .main-content > div:first-child {
